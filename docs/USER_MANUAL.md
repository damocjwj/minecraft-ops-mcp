...
MCP 服务先从 `http://` 或 `https://` URL 下载到临时文件，再上传到实例目录。默认最大文件大小是 256 MiB，可用 `max_bytes` 调整。
...
把实例中的单个文件下载到 MCP 服务所在机器。默认输出到 `/tmp/minecraft-ops-mcp-downloads/<文件名>`；如果指定 `local_path` 且文件已存在，默认会拒绝覆盖。
