...
- 明确依赖：运行时依赖 `mcp` 和 `websocket-client`，其余后端适配仍尽量保持轻量。
...
- `upload_local_file()` -> daemon `/upload/{password}`
- `upload_url_file()` -> 先下载 URL 到临时文件，再复用 `upload_local_file()`
- `prepare_download()` -> `POST /api/files/download`
- `download_local_file()` -> daemon `/download/{password}/{filename}`，失败时回退 `/download/{password}`，写入 MCP 本机路径
...
3. 用标准库构造 `multipart/form-data`。
4. POST 到 daemon 的 `/upload/{password}`。
...
当前工具数为 71。新增工具应优先复用已有命名分层，只有直接映射后端协议的逃生口才保留 raw call/command。
...
7. `msmp.discover`
8. MSMP 读操作
9. MSMP 写操作，先 dry-run，再 confirm
